...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      ...
      if (!me.data?.userData) throw new Error("Failed to load user data");

      setCurrentUser({
        email: me.data.userData.email,
        uid: me.data.userData.id || firebaseRes.user.uid,
      });

      setUserData(me.data.userData);

      return me.data.userData;
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (firebaseUser) => {
      // Prevent conflict when login() is already controlling everything
      ...
      if (!firebaseUser) {
        // If user is on login page → keep empty state
        if (onLoginPage) {
          setCurrentUser(null);
          setUserData(null);
          setLoading(false);
        ...
        // No firebase user → ensure state cleared
        setCurrentUser(null);
        setUserData(null);
        setLoading(false);
        return;
      ...
      // User exists in Firebase → Load backend user
